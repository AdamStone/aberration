...
class SLM(object):
    ...
    def __init__(self, px, nx, ny, m_tel=2, correction_file=None, scale=254):
        ...
        scale : float
            ...
            between 0 and 2pi can be tuned within the 256 channels of the 
            bitmap. The value expected is the channel at which the phase shift 
            should correspond to 2pi, which may vary (consult SLM documentation).
        """   
        self.px = px
        self.nx = nx
        self.ny = ny
...
class Sample(object):
    ...
    def __init__(self, n_s, focal_depths):
        ...
        focal_depths : list
            List of intended focal depth values (in m) below the sample surface. 
            For example, to irradiate at focal depths of 250 micron intervals
            with aberration correction applied, use [250e-6, 500e-6, 750e-6, 1000e-6]. 
        """      
        self.n_s = n_s
...
class Slit(object):
    """
    Convenience class for passing slit beamshaping parameters to create_bitmaps.
    """
...
def create_bitmaps(lens, slm, sample, window=None, slit=None):
    ...
    for d_w in window.thicknesses:
        for d_s in sample.focal_depths:
            ...
            if d_w != 0:
                print 'Processing window thickness {} micron and sample depth {} micron...'.format(d_w*1e6, d_s*1e6)
            else:
            ...
            ab_corrected, ab_dist_corrected = calculate_phasemap(slm.px, slm.nx, slm.ny, lens.wavelength, 
                                                                      d_s, lens.f, lens.alpha, sample.n_s, 
                                                                      lens.n_a, d_w, window.n_w, slm.m_tel, 
                                                                      slm.correction_file, slm.scale, slit)
            
            if d_w != 0:
                filename = 'window{}_depth{}_z{}'.format(int(d_w*1e6), int(d_s*1e6), int(z*1e6))
            else:
            ...
            if ab_dist_corrected != None:
                try: 
                    save('bitmaps/aberration_distortion_corrected/'+filename, ab_dist_corrected)
                except:
...
if __name__ == '__main__':
    ...
    slm = SLM(px = 20e-6,                           # pixel size, in meters
              ...
              correction_file = None)               # filename of distortion correction bmp (if available)
    ...
    window = Window(n_w = 1.398673,                 # refractive index of window
                    thicknesses = [0, 1000e-6])     # window thicknesses, in meters
    
    # slit beamshaping (optional)                                     
