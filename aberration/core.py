...
def diff_theta(theta_1, d_f, d_s, f, n_s, n_a=1, d_w=0, n_w=1):
    ...
    and Newton's Method requires this derivative. This analytical expression
    ...
    theta_1 : float
        Incident angle (in rads) after correction
        corresponding to a given theta.
    ...
    return (
        (d_w*n_a*np.sin(theta_1)/np.sqrt(-n_a**2*np.sin(theta_1)**2 + n_w**2) +
         d_s*n_a*np.sin(theta_1)/np.sqrt(-n_a**2*np.sin(theta_1)**2 + n_s**2) -
         (d_f + d_w)*np.tan(theta_1))*np.sin(theta_1)/f -
        (d_w*n_a**3*np.sin(theta_1)**2*np.cos(theta_1)/(-n_a**2 *
         np.sin(theta_1)**2 + n_w**2)**(3./2) + d_s*n_a**3*np.sin(theta_1)**2 *
         np.cos(theta_1)/(-n_a**2*np.sin(theta_1)**2 + n_s**2)**(3./2) +
         d_w*n_a*np.cos(theta_1)/np.sqrt(-n_a**2*np.sin(theta_1)**2 + n_w**2) +
         d_s*n_a*np.cos(theta_1)/np.sqrt(-n_a**2*np.sin(theta_1)**2 + n_s**2) -
         (np.tan(theta_1)**2 + 1)*(d_f + d_w))*np.cos(theta_1)/f)/np.sqrt(
        -(d_w*n_a*np.sin(theta_1)/np.sqrt(-n_a**2*np.sin(theta_1)**2 +
          n_w**2) + d_s*n_a*np.sin(theta_1)/np.sqrt(-n_a**2*np.sin(theta_1)**2
          + n_s**2) - (d_f + d_w)*np.tan(theta_1))**2 *
        np.cos(theta_1)**2/f**2 + 1) - 1
...
def PHI(theta, d_s, d_f, f, n_s, n_a=1, d_w=0, n_w=1):
    """
    Calculates total optical path length (geometric distance of path
    multiplied by refractive index of the medium) from a point on the
    lens (specified by theta) to the intended focal point (after correction).
    ...
    if theta != 0:
        ...
        ans = (n_a*ABpCD(theta, theta_1, d_f, f, d_w) +
               n_s * DE(d_s, theta_2) + n_w*BC(d_w, theta_3))
    else:
        ans = n_a*(f-d_f-d_w) + n_s*d_s + n_w*d_w
    return ans


def DeltaPHI(wavelength, theta, d_s, d_f, f, n_s,
             n_a=1, d_w=0, n_w=1, wrap=False):
    ...
    ret : float
        Value of the optical path length difference (in radians)
        for the given theta.
    """

    ans = -(PHI(theta, d_s, d_f, f, n_s, n_a, d_w, n_w) -
            PHI(0, d_s, d_f, f, n_s, n_a, d_w, n_w))
...
def calculate_phasemap(px, nx, ny, wavelength, d_s, f, alpha, n_s, n_a=1,
                       d_w=0, n_w=1, m_tel=2, correction_file=None,
                       scale=254, slit=None, wrap=True):
    """
    Calculates the aberration correction hologram for a given focal
    condition, for use with a digital SLM.
    ...
    if not slit:
        ...
        lower = np.random.rand(vmargin, nx)*2*np.pi
        if slit.orientation == 'x':
            middle = np.hstack((left_mid, NSWE[slit_margin:-slit_margin],
                                right_mid))
        if slit.orientation == 'y':
            middle = np.hstack((left_mid, NSWE[:, slit_margin:-slit_margin],
                                right_mid))
    ...
    if correction_file:
        ...
        corrected = np.array(np.round((scale/256.)*(final+correction)),
                             dtype='uint8')
    else:
        corrected = None
